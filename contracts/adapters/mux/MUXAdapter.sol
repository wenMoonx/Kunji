...
import "./interfaces/IMUXAdapter.sol";
import "./interfaces/IMuxOrderBook.sol";
import "./interfaces/IMuxReader.sol";
import "./interfaces/IMuxRouter.sol";
import "./interfaces/IMuxVault.sol";
...
library MUXAdapter {
    ...
    address internal constant muxRouter =
        0xaBBc5F99639c9B6bCb58544ddf04EFA6802F4064;
    address internal constant muxPositionRouter =
        0xb87a436B93fFE9D75c5cFA7bAcFff96430b09868;
    ImuxVault internal constant muxVault =
        ImuxVault(0x489ee077994B6658eAfA855C308275EAd8097C4A);
    address internal constant muxOrderBook =
        0x09f77E8A13De9a35a7231028187e9fD5DB8a2ACB;
    address internal constant muxOrderBookReader =
        0xa27C20A7CF0e1C68C0460706bB674f98F362Bc21;
    address internal constant muxReader =
    ...
    /// @notice Gives approve to operate with muxPositionRouter
    /// @dev Needs to be called from wallet and vault in initialization
    function __initApproveGmxPlugin() external {
        ImuxRouter(muxRouter).approvePlugin(muxPositionRouter);
        ImuxRouter(muxRouter).approvePlugin(muxOrderBook);
    }
    ...
    */
    function _increasePosition(
        ...
        _checkUpdateAllowance(path[0], address(muxRouter), amountIn);
        uint256 executionFee = ImuxPositionRouter(muxPositionRouter)
            .minExecutionFee();
        ...
        if (isLong) {
            acceptablePrice = muxVault.getMaxPrice(indexToken);
        } else {
            acceptablePrice = muxVault.getMinPrice(indexToken);
        }

        (bool success, bytes memory data) = muxPositionRouter.call{
            value: executionFee
        }(
            abi.encodeWithSelector(
                ImuxPositionRouter.createIncreasePosition.selector,
                path,
                indexToken,
                amountIn,
                minOut,
    ...
    */
    function _decreasePosition(
        ...
        (bool success, bytes memory data) = muxPositionRouter.call{
            ...
            abi.encodeWithSelector(
                ImuxPositionRouter.createDecreasePosition.selector,
                path,
                indexToken,
                collateralDelta,
                sizeDelta,
    ...
    */
    function _createIncreaseOrder(
        ...
        uint256 executionFee = ImuxOrderBook(muxOrderBook).minExecutionFee();
        if (address(this).balance < executionFee)
            revert InsufficientEtherBalance();
        ...
        _checkUpdateAllowance(vars.path[0], address(muxRouter), vars.amountIn);

        (bool success, bytes memory data) = muxOrderBook.call{
            value: executionFee
        }(
            abi.encodeWithSelector(
                ImuxOrderBook.createIncreaseOrder.selector,
                vars.path,
                vars.amountIn,
                vars.indexToken,
                vars.minOut,
    ...
    */
    function _updateIncreaseOrder(
        ...
        if (isTraderWallet) {
            ...
            ImuxOrderBook.IncreaseOrder memory walletOrder = _getIncreaseOrder(
                traderWallet,
                orderIndexes[0]
            );
            ImuxOrderBook.IncreaseOrder memory vaultOrder = _getIncreaseOrder(
                usersVault,
                orderIndexes[1]
            );
        ...
        ImuxOrderBook(muxOrderBook).updateIncreaseOrder(
            orderIndex,
            sizeDelta,
            triggerPrice,
            triggerAboveThreshold
    ...
    */
    function _cancelIncreaseOrder(
        ...
        ImuxOrderBook(muxOrderBook).cancelIncreaseOrder(orderIndex);
        return (true, ratio_);
    }
    ...
    */
    function _createDecreaseOrder(
        ...
        (bool success, bytes memory data) = muxOrderBook.call{
            ...
            abi.encodeWithSelector(
                ImuxOrderBook.createDecreaseOrder.selector,
                indexToken,
                sizeDelta,
                collateralToken,
                collateralDelta,
    ...
    */
    function _updateDecreaseOrder(
        ...
        if (isTraderWallet) {
            // calculate ratio for UserVault based on sizes of current orders
            ImuxOrderBook.DecreaseOrder memory walletOrder = _getDecreaseOrder(
                traderWallet,
                orderIndexes[0]
            );
            ImuxOrderBook.DecreaseOrder memory vaultOrder = _getDecreaseOrder(
                usersVault,
                orderIndexes[1]
            );
            ratio_ = vaultOrder.sizeDelta.mulDiv(
        ...
        ImuxOrderBook(muxOrderBook).updateDecreaseOrder(
            orderIndex,
            collateralDelta,
            sizeDelta,
            triggerPrice,
    ...
    */
    function _cancelDecreaseOrder(
        ...
        ImuxOrderBook(muxOrderBook).cancelDecreaseOrder(orderIndex);
        return (true, ratio_);
    }
    ...
    function _validateIncreaseOrder(
        ...
        uint256 latestIndex = ImuxOrderBook(muxOrderBook).increaseOrdersIndex(
            account
        );
        if (latestIndex >= 10) {
            uint256 tenthIndex = latestIndex - 10;
            ImuxOrderBook.IncreaseOrder memory order = ImuxOrderBook(
                muxOrderBook
    ...
    function _getPosition(
        ...
        return
            ImuxReader(muxReader).getPositions(
                address(muxVault),
                account,
                collaterals,
                indexTokens,
    ...
    function _getIncreaseOrder(
        address account,
        uint256 index
    ) internal view returns (ImuxOrderBook.IncreaseOrder memory) {
        return ImuxOrderBook(muxOrderBook).increaseOrders(account, index);
    }

    function _getDecreaseOrder(
        address account,
        uint256 index
    ) internal view returns (ImuxOrderBook.DecreaseOrder memory) {
        return ImuxOrderBook(muxOrderBook).decreaseOrders(account, index);
    }
